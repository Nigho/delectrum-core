Brewhaust Core version 3.0.2 is now available from:
...
Brewhaust Core v3.0.2 is a recommended, semi-mandatory update for all users. This release contains transaction creation bug fixes for zDLT spends, automint calculation adjustments, and other various updates/fixes.
...
Users that wish to prevent this behavior (not recommended) can pass `-backupzdlt=0` at the command-line when starting the client, or add `backupzdlt=0` to their `brewhaust.conf` file.
